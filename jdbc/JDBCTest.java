...
public class JDBCTest {
	...
	public static void main(String[] args) {
		/*
		 * Connection connection = null; try { connection = getConnection();
		 * System.out.println(connection); } catch (ClassNotFoundException |
		 * SQLException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); } finally { if (connection != null) try {
		 * connection.close(); } catch (SQLException e) { // TODO Auto-generated
		 * catch block e.printStackTrace(); } } insert(new Person("Aastha", 22,
		 * Gender.female)); insert(new Person("Abhi", 21, Gender.male));
		 */

		myPrepStat(new Person("Ajay", 23, Gender.male));
		getAllPerson();
	...
	public static void myPrepStat(final Person person) {

		Connection connection = null;
		final String querystr = "INSERT into PERSON (name,age) values (?,?)";
		System.out.println(querystr);
		// ResultSet rs= ps.executeQuery();
		try {
			connection = getConnection();
			PreparedStatement ps = (PreparedStatement) connection.prepareStatement(querystr);
			ps.setString(1, "Aastha");
			ps.setInt(2, 16);
...
//
	public static void myCallableStat() {
